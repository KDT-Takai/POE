...
// stored anywhere -- (col, row) is procedurally mapped to a screen position, and only
// which coordinates are *completed* is persisted. This is what makes the grid
...
public:
    ...
    // Distance from the start node (Chebyshev) -- purely a progress/depth measure now.
    // Nodes no longer have their own tier: the map's tier is whatever Waystone the player
    // drops into the socket (see AtlasSystem::TryPlaceWaystone / AI/DECISIONS.md), not a
    // property of the node itself.
    static int DistanceFromStart(int col, int row) {
        return (std::max)(std::abs(col), std::abs(row));
    }
