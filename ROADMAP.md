...
- インベントリ管理UI (`InventorySystem`, Iキーで開閉): 拾得アイテムは`InventoryComponent`(24枠)へ格納。Up/Downで選択、Enterで装備(既存装備は同じ枠に戻る=入れ替え)、Xで売却、Delで破棄に加え、マウスでもリスト行クリックで選択・下部のEquip/Sell/Discardボタンクリックで各操作を実行可能(キーボード操作と併存、パッシブツリーと同じ配色/ホバー方式)。選択中アイテムと該当スロットの現装備のPowerScoreを比較表示。満杯時は自動売却にフォールバック。セーブ/ロード対応 (`CampaignManager`)。
