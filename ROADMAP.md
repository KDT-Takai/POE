...
1. **実機での目視検証** — 今セッション後半 (アイテム/装備システム以降、インベントリUI・パッシブツリーUI含む) は分離テスト・ビルド成功のみでの検証。実際にプレイしての確認が必要。
2. **NPC/Vendor(商人)システム** — タウンでの購入/売却が未実装 (ゴールドは貯まるが使い道がまだ無い)。
3. **サウンド** — 効果音・BGMが `test.mp3` 以外未整備 (アセット不足によりブロック中)。
4. **スキル/モンスターバリエーション拡充** — 現状のスキル数・敵種類は最小限。
5. **原因不明の間欠的クラッシュの再調査** — セッション前半で発生し、ASan 6分ストレステストでは未検出。実機デバッガでの再現待ち。
6. **パッシブツリーの respec (再割り振り)** — 現状は一度割り振ったノードを戻す手段がない。PoEのオーブのような専用アイテムでの解放を検討。
